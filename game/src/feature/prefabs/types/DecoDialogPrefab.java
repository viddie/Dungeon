...
public final class DecoDialogPrefab extends Prefab {
  ...
  public void renderEditorFeedback(
    ...
    if (selected) {
      feedback.label(
          position.translate(0, 0.6f),
          value(instance, DECO)
              + " × "
              + value(instance, SCALE).x()
              + " × "
              + value(instance, SCALE).y());
      feedback.label(position.translate(0, 0.35f), value(instance, ONCE) ? "Dialog: once" : "Dialog");
      feedback.label(position.translate(0, 0.1f), value(instance, TEXT));
    }
  }
}
